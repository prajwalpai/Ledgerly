...
export function DocumentsClient({ initialDocuments, categories, accounts }: { initialDocuments: Document[]; categories: Definition[]; accounts: Definition[] }) {
  ...
  async function syncDrive() {
    ...
    if (response.ok) { const state = await fetch("/api/state", { cache: "no-store" }).then((item) => item.json()); setDocuments(state.documents); setMessage(`Drive checked: ${result.stored ?? 0} stored, ${result.review ?? 0} needing review, ${result.skipped ?? 0} skipped.`); }
